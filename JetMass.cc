...
class JetMass : public MC_JetAnalysis {
public:

	JetMass()
	: MC_JetAnalysis("JetMass", 4, "Jets")
	{	}
	...
	void analyze(const Event& event) {		
		...
		const Jets &jetlist = apply<FastJets>(event, "Jets").jetsByPt(Cuts::abseta < 2.8 && Cuts::pT > 100*GeV);

		if(jetlist.size() == 0) vetoEvent;

		for(const Jet& j : jetlist) {
			_file<<j.mass()<<"      "<< event.weights()[0] <<endl;
		}
